...
//	this->myFilter.setPackageFilter(MAVLINK_MSG_ID_STATUSTEXT, HANDLE);
 ...
 void DeviceServer::handleIncommingPackage(mavlink_message_t &msg){
	...
	}else if(msg.msgid == MAVLINK_MSG_ID_STATUSTEXT){ // lets get the time since boot from this messages since it is very freqent)
	...
	switch(this->state)
	{
	  ...
	  case GET_VERSION: 		
	  {
		if(msg.msgid == MAVLINK_MSG_ID_AUTOPILOT_VERSION){
			...
			printf("AutoPilot version: capabilities=%d uid=%d flight_sw_version=%d middleware_sw_version=%d os_sw_version=%d board_version=%d vendor_id=%d product_id=%d\n", 
			...
			if(this->droneParameters->getParameterMode() == UPDATE){
				...
				printf("Using cached mission and prameters, thus goint to RUNNING mode now\n");					

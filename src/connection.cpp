...
void Connection::initConnection(int port, int type, int flags){
 ...
 int16_t Connection::readData(void *buffer, uint16_t maxLength){ // returns number of bytes read.
	...
	// this->print_ipv4((struct sockaddr*)&this->_cliaddr);
	 ...
	 if(n==0){ // reading 0 means socket is closed, and thus we must close it.
   	    printf("Connection: Socket closed by user.\n");
		close(this->_fd);
		this->_fd=0;
		this->isValid=false;	 
	 }else if(n < 0){ // Error or not available (in none blocking mode).
